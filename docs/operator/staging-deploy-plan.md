...
Anvil's production runs on **<node-a> + <node-b>** (dual-node mesh), both currently on v2.3.2. This plan validates v3.0.0 on a separate staging VPS first, then performs an in-place upgrade on production.
...
- **Phase B — In-place upgrade on <node-a> + <node-b>** validates that the v2 → v3 migration runs cleanly on real production-shaped data + that the upgraded nodes find each other on the dual-node mesh via canonical federation.
...
2. **Transfer binary + example config to staging VPS — SFTP only**:
   ...
   tar -czf <sftp-staging-dir>/anvil-v3.0.0-staging-$(date +%Y%m%d).tar.gz \
...
## Phase B — In-place upgrade on <node-a> + <node-b>

### Prerequisites

- Phase A passed and was stable for at least 24 hours
- A 1-hour maintenance window for each production node
- Backup of `/var/lib/anvil/overlay` on both <node-a> and <node-b> (we will create one during the upgrade anyway, but a pre-existing operational snapshot is best practice)

### Sequence

Upgrade <node-a> first, validate, then <node-b>. Never both simultaneously — the mesh keeps at least one node serving traffic at all times.
...
3. **Transfer + replace the binary**. Same SFTP-only convention as Phase A — `scp` is blocked. If Phase A already transferred a tarball to this VPS, skip the local + SFTP steps and jump to the extract step.
   ...
   Local (only if <node-a>/<node-b> aren't already on the same VPS as staging):

   ```bash
   mkdir -p /tmp/anvil-v3-prod
   cp dist/anvil-linux-amd64 /tmp/anvil-v3-prod/anvil
   tar -czf <sftp-staging-dir>/anvil-v3.0.0-prod-$(date +%Y%m%d).tar.gz \
       -C /tmp/anvil-v3-prod .
   ```
   ...
   ```bash
   sftp <vps-user>@<node-a>-host
   sftp> cd /home/<vps-user>
   sftp> put anvil-v3.0.0-prod-YYYYMMDD.tar.gz
   sftp> bye
   # Repeat for <node-b>-host
   ```

   Robert (VPS via ssh) — extract + install:

   ```bash
   ssh <vps-user>@<node-a>-host
...
8. **Cross-mesh discovery check** — verify <node-a> sees <node-b> (and vice versa) via canonical SHIP/SLAP:
   ...
   curl -X POST http://localhost:9333/lookup \
     -H "Content-Type: application/json" \
     -H "x-topics: [\"ls_ship\"]" \
     -d '{"service":"ls_ship","query":{"domain":"https://<node-b>.your-domain.example"}}'
   ```

   Result: `output-list` with at least one entry (<node-b>'s SHIP ads).

   If empty after the first SyncAdvertisements cycle (give it 30 min after <node-b> is upgraded too): canonical discovery has a latency tail because both nodes need to publish and the trackers need to index. Worst-case wait: ~60 min after both nodes are up.

9. **Repeat for <node-b>**.

### Phase B acceptance

- [ ] Both <node-a> and <node-b> boot clean on v3.0.0
...
If Phase B fails on <node-a>, restore from snapshot + revert binary on that one node:
...
Do NOT upgrade <node-b> until you understand why <node-a> failed.

If Phase B fails on <node-b> after <node-a> is already on v3: roll BACK <node-b> first (per the rollback above), leave <node-a> on v3. The mesh continues to function in mixed mode because the legacy `/overlay/*` shim on v3 <node-a> accepts traffic from v2 <node-b> unchanged.
