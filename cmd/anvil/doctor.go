...
func runSelfHealingChecks(dataDir string, fix, yes bool) int {
	...
	// unlinked-file inodes alive for the running process, so <node-a> keeps
	...
	// wallets. Discovered in v3.0.0 production on <node-a> (443 MB
...
func applyHeaderRebuildAndVerify(dataDir string, svcs []diagnostics.ServiceState, preLagSecs int) error {
	...
	if !apiReady {
		return fmt.Errorf("post-wipe daemon never bound its HTTP listener — check journalctl -u <node-a>")
	}
